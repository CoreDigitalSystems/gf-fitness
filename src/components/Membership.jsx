import { IconCheck, IconTarget, IconArrow } from './Icons';
import { SectionLabel, SectionHeading, BtnFilled, BtnOutline } from './Primitives';
...
const TierCard = ({ t }) => (
  <div className={`relative bg-[#0a0a0a] border border-white/10 p-8 lg:p-10 flex flex-col ${
    t.featured ? 'glow lg:-my-4 lg:py-14' : ''
  }`}>
    {t.featured && (
      <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-[#4B5EFF] text-white text-[10px] font-mono uppercase tracking-[0.3em] px-4 py-1.5">
        Most Popular
      </div>
    )}
    <div className="flex items-baseline justify-between mb-1">
      <h3 className="font-display text-white text-2xl">{t.name}</h3>
      {t.featured && <IconTarget className="h-5 w-5 text-[#4B5EFF]" />}
    </div>
    ...
    <ul className="space-y-3.5 mb-10 flex-1">
      {t.features.map((f) => (
        <li key={f} className="flex items-start gap-3 text-[#cccccc] text-sm">
          <span className="mt-0.5 h-5 w-5 rounded-full bg-[#4B5EFF]/15 border border-[#4B5EFF]/40 flex items-center justify-center shrink-0">
            <IconCheck className="h-3 w-3 text-[#4B5EFF]" />
          </span>
          {f}
        </li>
      ))}
    ...
    {t.featured ? (
      <BtnFilled className="w-full">Join Now <IconArrow className="h-4 w-4" /></BtnFilled>
    ) : (
      <BtnOutline className="w-full">Join Now <IconArrow className="h-4 w-4" /></BtnOutline>
    )}
  </div>
);
  ...
  <section id="membership" className="bg-black py-24 lg:py-32 relative">
    <div className="max-w-[1440px] mx-auto px-6 lg:px-10">
      <div className="text-center max-w-2xl mx-auto mb-14">
        <SectionLabel color="text-[#4B5EFF]">04 · Membership</SectionLabel>
        <SectionHeading>
